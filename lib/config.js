...
const defaultConfig = {
  ...
  auth: {
    ...
    siteId: ''  // Will be auto-generated if not provided
  },
...
const notionInspiredPreset = {
  ...
  auth: {
    ...
    siteId: process.env.DOC_SITE_ID || ''  // Will be auto-generated if not provided
  },
  ...
  if (!fs.existsSync(docsPath)) {
    ...
    if (fs.existsSync(privatePath) && fs.statSync(privatePath).isDirectory()) {
      ...
      if (!config.auth.siteId) {
        config.auth.siteId = sharedAuth.generateSiteId(config.siteName);
        console.log(chalk.blue(`📋 Generated site ID: ${config.auth.siteId}`));
        console.log(chalk.yellow('   Note: Add this site ID to your Supabase database to enable user access'));
      }
    }
  }
